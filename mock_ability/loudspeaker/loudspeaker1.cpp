...
std::string abilityName = "loudspeaker1";
...
// 控制属性
std::string play = "off";
std::string url = "http://music.163.com/song/media/outer/url?id=447925558.mp3";
...
void handleDesire(const AbilityMessage::AbilityCommand &cmd) {
    for(const auto& item : cmd.desire) {
        if (item.controlName == "play") {
            LOG(INFO) << "设置play为: " << item.controlIntent;
            play = item.controlIntent;
        } else if (item.controlName == "url") {
            LOG(INFO) << "设置url为: " << item.controlIntent;
            url = item.controlIntent;
        }
    }
}
...
std::string handleStatus() {
    ...
    status.state = state;
    status.control["play"] = play;
    status.control["url"] = url;
