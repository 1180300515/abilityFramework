...
std::string abilityName = "motionTrigger1";

// 生命状态
std::string state = "INIT";

// 控制属性
std::string trigger = "off";
...
void handleDesire(const AbilityMessage::AbilityCommand &cmd) {
  for(const auto& item : cmd.desire) {
    if (item.controlName == "trigger") {
      DLOG(INFO) << "设置trigger为: " << item.controlIntent;
      trigger = item.controlIntent;
    }
  }
}
...
std::string handleStatus() {
  ...
  status.state = state;
  status.control["trigger"] = trigger;
