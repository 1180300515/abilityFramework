...
class AbilityImpl : public Ability::Service {
    Status Start(ServerContext *context, const StartInfo *request, Response *response) override
    {
        cout << "Start" << endl;
        if (GLOBAL_STATUS == STATUS_INIT || GLOBAL_STATUS == STATUS_SUSPEND) {
            cout << "request time: " << request->timestamp() << endl;
            cout << "time0: " << time(0) << endl;
            if (time(0) - request->timestamp() > 10 || request->timestamp() > time(0)) {
    ...
    Status Connect(ServerContext *context, const ConnectInfo *request, Response *response) override
    {
        cout << "Connect" << endl;
        if (GLOBAL_STATUS == STATUS_STANDBY || GLOBAL_STATUS == STATUS_SUSPEND) {
            if (time(0) - request->timestamp() > 10 || request->timestamp() > time(0)) {
                cout << "request timestamp error: "
                     << "Now time is : " << time(0) << ". Request time is :" << request->timestamp() << endl;
                response->set_code(RESPONSE_ERROR);
                response->set_msg("request timestamp error");
            } else {
                if (request->port() != 0) {
    ...
    Status Disconnect(ServerContext *context, const DisconnectInfo *request, Response *response) override
    {
    ...
    Status Terminate(ServerContext *context, const TerminateInfo *request, Response *response) override
    {
        cout << "Terminate" << endl;
        if (GLOBAL_STATUS == STATUS_RUNNING || GLOBAL_STATUS == STATUS_STANDBY || GLOBAL_STATUS == STATUS_SUSPEND) {
            if (time(0) - request->timestamp() > 10 || request->timestamp() > time(0)) {
                cout << "request timestamp error" << endl;
                response->set_code(RESPONSE_ERROR);
                response->set_msg("request timestamp error");
