...
int retriveOtherAudioParameters(snd_pcm_stream_t stream_type,
                                AudioHardwareForTest *audioDevice) {
  snd_pcm_t *pcm;
...
std::vector<AudioHardwareForTest> getAudioList(const std::string &type) {
  ...
  while (card >= 0) {
    ...
    if ((err = snd_ctl_open(&handle, name, 0)) < 0) {
      std::cout << "control open " << card << " error: " << snd_strerror(err)
                << std::endl;
      goto next_card;
    }
    if ((err = snd_ctl_card_info(handle, info)) < 0) {
      std::cout << "control hardware info " << card
    ...
    while (1) {
      ...
      if ((err = snd_ctl_pcm_info(handle, pcminfo)) < 0) {
        if (err != -ENOENT)
          std::cout << "control digital audio info (" << card
      ...
      AudioHardwareForTest audioDevice(name, snd_pcm_info_get_name(pcminfo), {},
                                       {}, {}, std::to_string(card),
                                       std::to_string(dev));
