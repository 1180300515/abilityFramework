...
int main(int argc, char **argv) {
  ...
  test->AddAbilityInstance("resource/resource_file/ability/camera_ability.yaml",
                           true);
  test->AddDeviceInstance("resource/resource_file/device/camera_instance1.yaml",
                          true);
  test->AddDeviceInstance("resource/resource_file/device/camera_instance2.yaml",
                          true);
  test->AddDeviceInstance("resource/resource_file/device/sensor_instance1.yaml",
                          true);
  test->AddDeviceInstance(
      "resource/resource_file/device/loudspeaker_instance1.yaml", true);

  test->Run();
  std::cout << RED << "打印当前instance" << NONE << std::endl;
  test->Print();

  std::cout << RED << "测试1 : 请插入camera , 并等待硬件刷新 , 刷新后可继续"
            << NONE << std::endl;
  block();
  ...
  std::cout << RED << "测试2 : 请拔出camera , 并等待硬件刷新 , 刷新后可继续"
            << NONE << std::endl;
  block();
  std::cout << RED << "打印所有instance信息" << NONE << std::endl;
  test->Print();
  block();
  std::cout << RED << "拉取相机信息 , 观察是否已经清除硬件信息" << NONE
            << std::endl;
