...
bool DatabaseManager::RegistCrdFromFile(const std::string &filepath) {
  ...
  std::string sql =
      "INSERT OR IGNORE INTO CRD (KEY,VALUE,APIGROUP,KIND,SCHEMA) VALUES (\'";
  sql += crd_key_name + "\',\'" + crd_json_string + "\',\'" + crd_group_string +
         "\',\'" + crd_kind_string + "\',\'" + crd_schema_string + "\' );";
  int rc;
  char *erroms = 0;
...
bool DatabaseManager::DBGetDeviceInstances(
    const std::string &resourcetype,
    std::map<std::string, std::string> *instance) {
  devicestructs.clear();
  std::string search_sql =
      "SELECT * FROM DEVICE WHERE KIND = \'" + resourcetype + "\' ;";
  int rc;
  char *erroms = 0;
...
bool DatabaseManager::DBGetAbilityInstances(
    std::map<std::string, std::string> *instance) {
...
bool DatabaseManager::DBUpdateDeviceInstance(const Json::Value &jnode) {
  ...
  std::string update_sql = "UPDATE DEVICE SET VALUE = \'" + instance_value +
                           "\' , APIGROUP = \'" + instance_group +
                           "\', KIND = \'" + instance_kind +
...
bool DatabaseManager::DBUpdateAbilityInstance(
    const Json::Value &instance_json) {
  ...
  if (instance_json["metadata"].isMember("namespace")) {
    std::string instance_namespace =
        instance_json["metadata"]["namespace"].asString();
  }
  std::string instance_key = instance_namespace + "/" + instance_name;
  std::string update_sql = "UPDATE ABILITY SET VALUE = \'" + instance_value +
                           "\' , APIGROUP = \'" + instance_group +
                           "\', KIND = \'" + instance_kind +
                           "\', VERSION = \'" + instance_version;
  update_sql += "\' WHERE KEY = \'" + instance_key + "\' ;";
...
int DatabaseManager::crd_callback(void *unused, int columenCount,
...
int DatabaseManager::device_callback(void *unused, int columenCount,
                                     char **columnValue, char **columnName) {
...
int DatabaseManager::ability_callback(void *unused, int columenCount,
                                      char **columnValue, char **columnName) {
  InstanceDBStruct instance;
  for (int i = 0; i < columenCount; i++) {
    if (strcmp(columnName[i], "KEY") == 0) {
...
bool DatabaseManager::storeDevice(const Json::Value &instance_json) const {
  ...
  std::string insert_sql =
      ...
      "(\'";
  insert_sql += instance_key + "\',\'" + instance_value + "\',\'" +
                instance_group + "\',\'" + instance_kind + "\',\'" +
                instance_version + "\' );";
  int rc2;
  char *erroms2 = 0;
