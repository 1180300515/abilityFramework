...
bool ResourceManager::AddAbilityInstance(const std::string &data,
                                         bool from_file) {
...
bool ResourceManager::AddDeviceInstance(const std::string &data,
                                        bool from_file) {
  Json::Value instance_json;
  if (from_file) {
    YAML::Node instance_yaml;
...
void ResourceManager::EndAddressDiscoveryResult(
...
void ResourceManager::LoadLocalResourceFromDB() {
  ...
  DatabaseManager::getInstance().DBGetDeviceInstances(
      MicrophoneDeviceResourcetype, &data);
  if (data.size() != 0) {
    for (const auto &iter : data) {
  ...
  data.clear();
  DatabaseManager::getInstance().DBGetDeviceInstances(
      LoudspeakerDeviceResourcetype, &data);
  if (data.size() != 0) {
    for (const auto &iter : data) {
...
void ResourceManager::Run() {
  autoGenerateCR();
  periodic_scan_thread =
      std::thread(&ResourceManager::periodicScanThread, this);
}
...
std::shared_ptr<DeviceInstanceInfo> ResourceManager::GetDeviceInstance(
...
std::shared_ptr<AbilityInstanceInfo> ResourceManager::GetAbilityInstance(
    const std::string &key) {
  std::lock_guard<std::mutex> locker(abilities_lock_);
  if (abilities_.count(key) != 0) {
...
std::vector<std::string> ResourceManager::GetHardWareResourceList(
    std::string type) {
