 ...
 *
 * File: hardware_scan.cpp
 * Description: hardware_scan.cpp
 * Version: V1.0.0
 * Date: 2023/08/24
...
void HardwareScan::getMicInfo() { this->profile.micDevices = getAudioList("record"); }

void HardwareScan::getSpeakerInfo() { this->profile.speakerDevices = getAudioList("playback"); }
...
std::vector<AudioHardware> HardwareScan::getAudioList(const std::string &type)
{
    ...
    while (card >= 0) {
        ...
        if ((err = snd_ctl_card_info(handle, info)) < 0) {
            DLOG(ERROR) << "control hardware info " << card << " error: " << snd_strerror(err);
        ...
        while (1) {
            ...
            if ((err = snd_ctl_pcm_info(handle, pcminfo)) < 0) {
                if (err != -ENOENT) DLOG(ERROR) << "control digital audio info (" << card << "): " << snd_strerror(err);
                continue;
            }
            std::string name;
            if (type == "record") {
...
int HardwareScan::retriveOtherAudioParameters(snd_pcm_stream_t stream_type, AudioHardware *audioDevice)
{
    snd_pcm_t *pcm;
...
void HardwareScan::UnmatchedHardware(std::vector<CameraHardware> *camera, std::vector<AudioHardware> *mic, std::vector<AudioHardware> *speaker)
{
    std::lock_guard<std::mutex> locker1(this->map_lock_);
    ...
    for (const auto &hw : profile.cameraDevices) {
        if (hardware_instance_map.count(hw.bus_info) != 0 && hardware_instance_map.at(hw.bus_info) == "") {
            camera->emplace_back(hw);
        }
    }
    for (const auto &hw : profile.micDevices) {
        if (hardware_instance_map.count(hw.name) != 0 && hardware_instance_map.at(hw.name) == "") {
            mic->emplace_back(hw);
        }
    }
    for (const auto &hw : profile.speakerDevices) {
...
void HardwareScan::LocalHardwareScan()
{
    ...
    getSpeakerInfo();
    getDisplayHardware();
    DLOG(INFO) << "hardware scan finish : camera: " << this->profile.cameraDevices.size() << "  mic: " << this->profile.micDevices.size()
               << "  speaker: " << this->profile.speakerDevices.size() << "  display: " << this->profile.displayDevices.size();
    compareMap();
}
...
std::optional<Json::Value> HardwareScan::GetHardware(const std::string &id, const std::string &key, const std::string &type)
{
    if (id == "") {
        return std::nullopt;
    }
    ...
    if (type == CameraDeviceResourcetype) {
        ...
        for (const auto &iter : profile.cameraDevices) {
            if (iter.bus_info == id) {
                ...
                if (key_in_map != "" && key_in_map != key && !isAutogenInstance(key_in_map)) {
                    DLOG(ERROR) << "the camera hardware: \"" << id << "\" already match with: " << key_in_map;
                    return std::nullopt;
                } else {
                    return iter.toJson();
                }
        ...
        for (const auto &iter : profile.speakerDevices) {
            if (iter.name == id) {
                auto key_in_map = hardware_instance_map.at(id);
                if (key_in_map != "" && key_in_map != key && !isAutogenInstance(key_in_map)) {
                    DLOG(ERROR) << "the speaker hardware: \"" << id << "\" already match with: " << key_in_map;
                    return std::nullopt;
                } else {
                    return iter.toJson();
                }
        ...
        for (const auto &iter : profile.micDevices) {
            if (iter.name == id) {
                auto key_in_map = hardware_instance_map.at(id);
                if (key_in_map != "" && key_in_map != key && !isAutogenInstance(key_in_map)) {
                    DLOG(ERROR) << "the mic hardware: \"" << id << "\" already match with: " << key_in_map;
                    return std::nullopt;
                } else {
                    return iter.toJson();
                }
