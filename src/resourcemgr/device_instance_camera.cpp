  ...
  if (hardware.bus_info == spec.hardwareidentifier) {
    ...
    if (hardware.formats.size() == spec.properties.supportFormat.size()) {
      for (const auto &iter : hardware.formats) {
        auto count = std::count(spec.properties.supportFormat.begin(),
                                spec.properties.supportFormat.end(), iter);
        if (count == 0) {
...
Json::Value CameraInstance::ToJson() const {
  ...
  for (int i = 0; i < spec.properties.supportFormat.size(); i++) {
    jnode["spec"]["properties"]["supportFormat"].append(
        spec.properties.supportFormat[i]);
  }
...
bool CameraInstance::FromJson(const Json::Value &jnode) {
  ...
  spec.properties.resolution =
      jnode["spec"]["properties"]["resolution"].asString();
  spec.properties.location = jnode["spec"]["properties"]["location"].asString();
  spec.properties.wideAngle =
      std::stoi(jnode["spec"]["properties"]["wideAngle"].asString());
  spec.properties.focusMethod =
      jnode["spec"]["properties"]["focusMethod"].asString();
  spec.properties.telephoto =
      (jnode["spec"]["properties"]["telephoto"].asString() == "true");
  spec.properties.devicePath =
      jnode["spec"]["properties"]["devicePath"].asString();
  spec.properties.driverName =
      jnode["spec"]["properties"]["driverName"].asString();
  spec.properties.card = jnode["spec"]["properties"]["card"].asString();
  spec.properties.busInfo = jnode["spec"]["properties"]["busInfo"].asString();
  for (int i = 0; i < jnode["spec"]["properties"]["supportFormat"].size();
       i++) {
    spec.properties.supportFormat.emplace_back(
        jnode["spec"]["properties"]["supportFormat"][i].asString());
  }
