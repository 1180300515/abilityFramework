...
void HardwareResourceManager::Init(std::shared_ptr<ResourceManager> manager_,
                                   std::string hostname) {
  this->resource_manager = manager_;
  this->hostname_ = hostname;
}

void HardwareResourceManager::EndAddressResult(
...
std::vector<std::string> HardwareResourceManager::GetHardwareResourceList(
    std::string type) {
...
DeviceProfile HardwareResourceManager::getDeviceProfileFromHost(
    const std::string &ip) {
  ...
  if (res && res->status == 200) {
    ...
    bool parsingSuccessful = reader->parse(
        res->body.c_str(), res->body.c_str() + res->body.size(), &root, &errs);
...
void HardwareResourceManager::compareOldAndNew(
    std::map<std::string, std::string> new_) {
