 ...
 *
 * File: lan_ipv4_discovery.cpp
 * Description: lan_ipv4_discovery.cpp
 * Version: V1.0.0
 * Date: 2023/08/24
...
void LANIPV4Discovery::RunBroadcastReceiver() {
  this->receiver_thread =
      std::thread(&LANIPV4Discovery::udp_broadcast_receiver, this);
  DLOG(INFO) << L_GREEN << "start lan-ipv4-discovery broadcast receiver success"
            << NONE;
}
...
void LANIPV4Discovery::udp_broadcast_receiver() {
  ...
  while (true) {
    ...
    ssize_t len = recvfrom(sock, recv_buf, sizeof(recv_buf) - 1, 0,
    ...
    auto newdevice = DiscoveryDeviceInfo{hostname,  ip, this->discoverySource,
                                         timestamp, 0,  status};
    callback(newdevice);
  }
...
void LANIPV4Discovery::RegisterCallback(
    std::function<void(DiscoveryDeviceInfo)> callback) {
...
void LANIPV4Discovery::BroadcastSender() {
  ...
  std::string message = std::string(hostname) + "+" + loadavg + "+" +
                        std::to_string(std::time(nullptr));

  // DLOG(INFO) << L_PURPLE << "Broadcasting: " << message << NONE;
