...
void LANIPV4Discovery::RunBroadcastReceiver()
{
    ...
    DLOG(INFO) << L_GREEN << "start lan-ipv4-discovery broadcast receiver success" << NONE;
}
...
void LANIPV4Discovery::RegisterCallback(std::function<void(DiscoveryDeviceInfo)> callback) { this->callback = callback; }
