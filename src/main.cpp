...
int main(int argc, char **argv) {
  ...
  lifecycle_manager->Init(std::bind(&ResourceManager::AbilityExistJudge,
                                    resource_manager, std::placeholders::_1));
  discovery_manager->Init(std::bind(&ResourceManager::EndAddressDiscoveryResult,
                                    resource_manager, std::placeholders::_1));
  ability_relation_manager->Init(
      std::bind(&ResourceManager::GetAbilityInfoExtractList, resource_manager),
      std::bind(&ResourceManager::GetHardWareResourceList, resource_manager,
                std::placeholders::_1));
  http_server->Init(resource_manager, lifecycle_manager,
                    ability_relation_manager);
