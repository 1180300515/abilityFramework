 ...
 *
 * File: hardware_scan.h
 * Description: hardware_scan.h
 * Version: V1.0.0
 * Date: 2023/08/24
...
class HardwareScan {
   public:
    ...
    std::optional<Json::Value> GetHardware(const std::string &id, const std::string &key, const std::string &type);

    bool isAutogenInstance(const std::string &key);

    std::string GetMatchedKey(const std::string &id);

    void UnmatchedHardware(std::vector<CameraHardware> *camera, std::vector<AudioHardware> *mic, std::vector<AudioHardware> *speaker);
